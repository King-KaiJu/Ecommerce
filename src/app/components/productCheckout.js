...
const ProductCheckout = () =>{
    ...
    return (
      <section className='bg-slate-500 rounded-lg p-5'>
        <h1>Products</h1>
        <article className='bg-slate-800 rounded-lg p-5'>
          {cart.map(product => (
            <div key={product.id}>
        ...
        <div className='flex items-center justify-center'>
          <p className='text-cyan-300 bg-slate-800 text-xl p-5 my-4 rounded'>Total: € {(cartTotal()).toFixed(2)}</p>
        </div>
      </section>
    );
};
